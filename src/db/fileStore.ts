 ...
 * يتم دمج طلبات الحفظ المتزامنة (Debouncing) لحفظ أحدث حالة فقط.
 */
export function saveToFile(state: AppState): Promise<void> {
  ...
  if (!isWritePending) {
    ...
    const run = writeQueue.then(async () => {
      ...
      const content = JSON.stringify(fileCache); // Serialize the most recent state
      try {
        const dir = await resolveDataDir();
        if (dir) {
          await atomicWrite(dir, content);
        } else {
          await idbSet('state', fileCache);
        }
      } catch (err) {
        console.error('fileStore: فشلت الكتابة الذرية:', err);
        try { await idbSet('state', fileCache); } catch { /* تجاهل */ }
      }
    });
