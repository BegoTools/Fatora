...
export interface Item {
  ...
  isActive: boolean;
  createdAt: string;
  image?: string;       // base64/blob صورة الصنف (مخزّنة محلياً في الـ DB)
  // ── الضريبة (Tax) ──
...
export interface SaleInvoice {
  ...
  previousBalanceInvoiceRef?: string; // رقم آخر فاتورة مرتبطة بالرصيد السابق
...
export interface CompanySettings {
  ...
  logo?: string;
  ownerName1?: string;       // اسم المالك/الشريك الأول (يظهر في الفاتورة)
  ownerName2?: string;       // اسم المالك/الشريك الثاني (يظهر في الفاتورة)
  accentColor?: string;      // اللون الأساسي المخصّص للتطبيق (hex)
  // ── الضريبة (Tax) ──
...
export interface InvoiceSheetCellStyle {
  bold?: boolean;
  italic?: boolean;
  fontSize?: number;
  color?: string;
...
export interface InvoiceSheetCell {
  id: string;
  row: number;
  column: number;
  value: string;
  rowSpan?: number;
  columnSpan?: number;
  style?: InvoiceSheetCellStyle;
}

export interface InvoiceSpreadsheetLayout {
  columnWidths: number[];
  rowHeights: number[];
  cells: InvoiceSheetCell[];
}
...
export type InvoiceSectionId = 'header' | 'details' | 'items' | 'totals' | 'footer';
...
export interface InvoiceLayoutSettings {
  ...
  totalsLabelColor: string;
  /** Per-section presentation controls used by the visual invoice designer. */
  sectionStyles?: Partial<Record<InvoiceSectionId, {
    hidden?: boolean;
    width?: number;
    height?: number;
    padding?: number;
...
export interface InvoiceDesign {
  ...
  /** User-defined fields; kept separate from the original two fields for backwards compatibility. */
  customFields?: InvoiceCustomField[];
  spreadsheet?: InvoiceSpreadsheetLayout;
  footerTextEn: string;
  footerTextAr: string;
  thankYouEn: string;
  thankYouAr: string;
...
// ============================================================
// حركات حساب العميل (Customer Transactions) — لكشف الحساب
// ============================================================
export type CustomerTransactionType = 'invoice' | 'payment' | 'adjustment' | 'return' | 'exchange' | 'credit_note' | 'debit_note';
...
export interface CustomerTransaction {
  ...
  amount: number;
  direction: 'debit' | 'credit'; // debit = مدين (على العميل)، credit = دائن (له)
  reason?: string;               // سبب الحركة (إجباري للتعديل اليدوي)
  relatedInvoiceId?: string;     // ربط بفاتورة
  relatedInvoiceNumber?: string; // رقم الفاتورة للعرض
  createdAt: string;
  createdBy: string;
}
...
export interface AppState {
  ...
  customerTransactions?: CustomerTransaction[]; // حركات حساب العملاء
  
  // Warehouse State
  warehouses?: import('../services/warehouse').Warehouse[];
  stockTransfers?: import('../services/warehouse').StockTransfer[];
  bundleAssemblies?: import('../services/warehouse').BundleAssembly[];
}

// ============================================================
// قيود اليومية المحاسبية المزدوجة (Double-Entry Journal Entries)
// ============================================================
...
export interface JournalEntry {
  id: string;
  entryNumber: string;
  sourceType: 'sale' | 'purchase' | 'return' | 'payment' | 'payroll' | 'cash_audit' | 'manual' | 'maintenance';
  sourceId: string;
  date: string;
  description: string;
  lines: JournalLine[];
...
// ============================================================
// تهيئة وتخصيص القطاعات (Sector Profiles)
// ============================================================
export type SectorTypeId = 'general' | 'clothing' | 'supermarket' | 'electronics' | 'maintenance' | 'distribution' | 'cosmetics';
...
export interface SectorProfile {
  ...
  showWeightScaleBarcode: boolean;
  ...
  autoItemCoding?: boolean;
}

// ============================================================
// موديول الورش والصيانة (Maintenance & Workshop Receipts)
// ============================================================
export type MaintenanceStatus = 'received' | 'under_inspection' | 'ready' | 'delivered' | 'cancelled';
...
export interface MaintenanceReceipt {
  ...
  technicianCommission: number; // عمولة أو أجر الفني
...
// ============================================================
// جرد ومطابقة الخزينة (Cash Audit & Reconciliation)
// ============================================================
export interface CashAuditRecord {
  ...
  bookBalance: number;       // الرصيد الدفتري
  actualCashBalance: number; // الرصيد الفعلي في الصندوق
  difference: number;        // الفارق (موجب = زيادة، سالب = عجز)
  status: 'matched' | 'deficit' | 'surplus';
  adjustmentJournalId?: string; // معرّف القيد التسويي لـ postJournalEntry
  notes?: string;
  createdAt: string;
  createdBy: string;
}
