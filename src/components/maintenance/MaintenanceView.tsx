                  ...
                  {filteredReceipts.length === 0 ? (
                    ...
                    filteredReceipts.map(r => (
                      <tr key={r.id} className="hover:bg-gray-50/50 transition">
                        ...
                        <td className="p-4"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">{t(`maintenance.status.${r.status === 'under_inspection' ? 'inspecting' : r.status}`)}</span></td>
                        <td className="p-4">
                          <select
                            value={r.status}
            ...
            {technicianLedger.map(tech => (
              <div key={tech.technicianName} className="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-2">
                ...
                <div className="flex justify-between text-sm text-gray-600">
                  <span>{t('maintenance.totalDevices')}:</span>
                  <span className="font-semibold">{tech.totalJobs}</span>
                </div>
                <div className="flex justify-between text-sm text-gray-600">
                  <span>{t('maintenance.completedDevices')}:</span>
                ...
                <div className="flex justify-between text-sm pt-2 border-t border-gray-200 font-bold text-blue-700">
                  <span>{t('maintenance.totalDue')}:</span>
                  <span>{tech.totalCommissions} {t('common.currency')}</span>
                </div>
              </div>
            ))}
          </div>
