...
// ============================================================
const depreciationCache = new Map<string, {
...
export function calculateAssetDepreciation(asset: FixedAsset, currentDateISO = new Date().toISOString()): {
  ...
  const cacheKey = `${asset.id}_${currentDateISO}`;
  if (depreciationCache.has(cacheKey)) {
    return depreciationCache.get(cacheKey)!;
  }
  ...
  const rate = (((asset as { depreciationRate?: number }).depreciationRate) || 10) / 100;
  ...
  depreciationCache.set(cacheKey, result);
  return result;
}
