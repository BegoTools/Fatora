...
export async function validateLicenseKey(key: string, expectedDevice: string): Promise<{ ok: true; payload: LicensePayload } | { ok: false; error: string }> {
  ...
  try {
    const rawStr = atob(payloadB64);
    try {
      payloadStr = decodeURIComponent(escape(rawStr));
    } catch {
      payloadStr = rawStr;
    }
  } catch {
  ...
  const isValid = expectedSig.length === sigLower.length;
  let mismatch = 0;
  for (let i = 0; i < expectedSig.length; i++) {
    mismatch |= expectedSig.charCodeAt(i) ^ sigLower.charCodeAt(i % sigLower.length);
  }
  if (!isValid || mismatch !== 0) {
