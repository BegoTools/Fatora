...
type Action =
  ...
  | { type: 'ADD_CUSTOMER_TRANSACTION'; payload: CustomerTransaction }
  | { type: 'RESET_DEMO' };
...
export function recalcCustomerBalances(data: AppState): AppState {
  ...
  const add = (cid: string | null | undefined, amt: number) => {
    if (!cid) return;
    balances.set(cid, round2((balances.get(cid) ?? 0) + amt));
  };
...
function appReducer(state: AppContextState, action: Action): AppContextState {
  ...
  switch (action.type) {
    ...
    case 'ADD_SALE': {
      ...
      // Update treasury
      if (action.payload.paymentMethod !== 'credit' && action.payload.paid > 0) {
        const accTypeMap: Record<string, string> = { cash: 'safe', card: 'bank', wallet: 'wallet' };
        const targetType = accTypeMap[action.payload.paymentMethod] || 'safe';
        let account = newData.treasuryAccounts.find(a => a.type === targetType);
        if (!account) {
          console.warn(`Account type ${targetType} not found, falling back to safe`);
          account = newData.treasuryAccounts.find(a => a.type === 'safe');
        }
        if (account) {
          newData.treasuryAccounts = newData.treasuryAccounts.map(a =>
            a.id === account!.id ? { ...a, balance: a.balance + action.payload.paid } : a
          );
          // Add transaction
    ...
    case 'UPDATE_SALE': {
      ...
      if (oldInv) {
        ...
        if (oldInv.paymentMethod !== 'credit' && oldInv.paid > 0) {
          const accTypeMap: Record<string, string> = { cash: 'safe', card: 'bank', wallet: 'wallet' };
          const oldTargetType = accTypeMap[oldInv.paymentMethod] || 'safe';
          const oldAccount = newData.treasuryAccounts.find(a => a.type === oldTargetType) || newData.treasuryAccounts.find(a => a.type === 'safe');
          if (oldAccount) {
            newData.treasuryAccounts = newData.treasuryAccounts.map(a => a.id === oldAccount!.id ? { ...a, balance: a.balance - oldInv.paid } : a);
          }
        }
        if (action.payload.paymentMethod !== 'credit' && action.payload.paid > 0) {
          const accTypeMap: Record<string, string> = { cash: 'safe', card: 'bank', wallet: 'wallet' };
          const newTargetType = accTypeMap[action.payload.paymentMethod] || 'safe';
          const newAccount = newData.treasuryAccounts.find(a => a.type === newTargetType) || newData.treasuryAccounts.find(a => a.type === 'safe');
          if (newAccount) {
            newData.treasuryAccounts = newData.treasuryAccounts.map(a => a.id === newAccount!.id ? { ...a, balance: a.balance + action.payload.paid } : a);
          }
        }
      }
      newData = recalcCustomerBalances(newData);
    ...
    case 'DELETE_SALE': {
      ...
      if (inv) {
        ...
        if (inv.paymentMethod !== 'credit' && (inv.paid || 0) > 0) {
          const accTypeMap: Record<string, string> = { cash: 'safe', card: 'bank', wallet: 'wallet' };
          const targetType = accTypeMap[inv.paymentMethod] || 'safe';
          const account = newData.treasuryAccounts.find(a => a.type === targetType) || newData.treasuryAccounts.find(a => a.type === 'safe');
          if (account) {
            newData.treasuryAccounts = newData.treasuryAccounts.map(a => a.id === account!.id ? { ...a, balance: a.balance - inv.paid } : a);
          }
        }
      }
      newData = recalcCustomerBalances(newData);
    ...
    case 'ADD_PURCHASE': {
      ...
      // Deduct from treasury if paid > 0
      if (action.payload.paid > 0) {
        const safe = newData.treasuryAccounts.find(a => a.type === 'safe');
        if (safe) {
          newData.treasuryAccounts = newData.treasuryAccounts.map(a =>
            a.id === safe.id ? { ...a, balance: Math.max(0, a.balance - action.payload.paid) } : a
          );
          newData.transactions = [...newData.transactions, {
            id: generateId('tr'),
            accountId: safe.id,
            ...
            type: 'expense',
            amount: action.payload.paid,
            description: `Payment to ${action.payload.supplierName} — PO ${action.payload.invoiceNumber}`,
            referenceNumber: action.payload.invoiceNumber,
            category: 'Purchases',
            date: new Date().toISOString().split('T')[0],
            createdAt: new Date().toISOString(),
    ...
    case 'ADD_TRANSACTION': {
      ...
      if (account) {
        ...
        newData.treasuryAccounts = newData.treasuryAccounts.map(a =>
          a.id === account.id ? { ...a, balance: round2(a.balance + delta) } : a
        );
      }
      saveState(newData);
      return { ...state, data: newData };
    ...
    case 'ADD_MULTIPLE_TRANSACTIONS': {
      ...
      action.payload.forEach(tr => {
        const delta = tr.type === 'expense' || tr.type === 'transfer_out' ? -tr.amount : tr.amount;
        accountDeltas[tr.accountId] = (accountDeltas[tr.accountId] || 0) + delta;
      });
      newData.treasuryAccounts = newData.treasuryAccounts.map(a =>
        accountDeltas[a.id] !== undefined ? { ...a, balance: round2(a.balance + accountDeltas[a.id]) } : a
      );
      saveState(newData);
      return { ...state, data: newData };
    }
    ...
    case 'UPDATE_TRANSACTION': {
      const oldTr = state.data.transactions.find(t => t.id === action.payload.id);
      newData = { ...state.data, transactions: state.data.transactions.map(t => t.id === action.payload.id ? action.payload : t) };
      
      const accountDeltas: Record<string, number> = {};
      
      // Reverse old transaction impact
      if (oldTr) {
        const oldDelta = oldTr.type === 'expense' || oldTr.type === 'transfer_out' ? oldTr.amount : -oldTr.amount;
        accountDeltas[oldTr.accountId] = (accountDeltas[oldTr.accountId] || 0) + oldDelta;
      }
      
      // Apply new transaction impact
      const newDelta = action.payload.type === 'expense' || action.payload.type === 'transfer_out' ? -action.payload.amount : action.payload.amount;
      accountDeltas[action.payload.accountId] = (accountDeltas[action.payload.accountId] || 0) + newDelta;
    ...
    case 'ADD_CUSTOMER_TRANSACTION': {
      newData = {
        ...state.data,
        customerTransactions: [...(state.data.customerTransactions || []), action.payload],
      };
      saveState(newData);
      return { ...state, data: newData };
    }
