...
import { buildInvoicePrintHtml } from '@/components/invoice/invoicePrint';
import { normalizeInvoice } from '@/components/invoice/invoiceModel';
...
export function Sales() {
  ...
  const [lastInvoiceScope, setLastInvoiceScope] = useState<'global' | 'customer'>('global');
  ...
  const cartSubtotal = round2(cart.reduce((sum, ci) => sum + (ci.unitPrice * ci.quantity), 0));
  const cartDiscount = round2(cart.reduce((sum, ci) => sum + (ci.discount ?? 0), 0));
  const cartTotal = round2(cartSubtotal - cartDiscount + extraCharges.reduce((sum, ec) => sum + (ec.amount ?? 0), 0));
  ...
  const checkoutPayment = paymentMethod === 'card' || paymentMethod === 'wallet' ? cartTotal : paidAmount;
  ...
  const setDiscount = (itemId: string, value: number) => {
    const discount = Math.max(0, value || 0);
    setCart(cart.map(ci => (ci.item.id === itemId ? { ...ci, discount: Math.min(discount, ci.unitPrice) } : ci)));
  };
  ...
  const handleCheckout = () => {
    ...
    const previousInvoice = selectedCustomer
      ? [...salesInvoices]
        .filter(invoice => invoice.customerId === selectedCustomer && invoice.id !== editingInvoiceId && invoice.remaining > 0)
    ...
    if (remaining > 0 && cust && cust.creditLimit > 0 && remaining > cust.creditLimit) {
      toast.error(t('sales.creditLimitExceeded') || 'الرصيد المتبقي يتجاوز الحد الائتماني للعميل!');
      return;
    }
    ...
    if (editingInvoiceId && oldInv) {
      const updated: SaleInvoice = {
        ...
        remaining,
        previousBalance,
        previousBalanceInvoiceRef: previousInvoice?.invoiceNumber,
        downPayment: paid,
        netAmount: remaining,
        paymentMethod,
        paymentStatus,
    ...
    const newInvoice: SaleInvoice = {
      id: generateId('sale'),
      invoiceNumber: generateInvoiceNumber('INV', salesInvoices.map(i => i.invoiceNumber)),
      customerId: selectedCustomer || null,
      customerName,
      items,
      subtotal: cartSubtotal,
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'F2') {
        ...
        e.preventDefault();
        document.getElementById('pos-amount-paid-input')?.focus();
      } else if (e.key === 'F9') {
        e.preventDefault();
        handleCheckout();
      }
  ...
  }, [view, cart, selectedCustomer, amountPaid, paymentMethod, extraCharges, editingInvoiceId]);
  ...
  const handleCollectPayment = () => {
    ...
    const safe = state.data.treasuryAccounts.find(a => a.type === 'safe');
    if (safe) {
      dispatch({
        ...
        payload: {
          ...
          amount: collectValue,
          category: 'Sales',
          description: `Payment Collection — Invoice ${collectTargetInvoice.invoiceNumber}`,
          referenceNumber: collectTargetInvoice.invoiceNumber,
  ...
  const paginatedInvoices = filteredInvoices.slice().reverse().slice((currentPage - 1) * invoicesPerPage, currentPage * invoicesPerPage);

  // ─── آخر فاتورة بيع في النظام (لزر طباعة آخر فاتورة) ───
  const lastSaleInvoice = useMemo(() => {
    const scoped = lastInvoiceScope === 'customer' && selectedCustomer
      ? salesInvoices.filter(invoice => invoice.customerId === selectedCustomer)
      : salesInvoices;
    if (scoped.length === 0) return null;
    return [...scoped].sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0];
  }, [salesInvoices, lastInvoiceScope, selectedCustomer]);

  const printLastInvoice = () => {
    if (!lastSaleInvoice) return;
    const cust = customers.find(c => c.id === lastSaleInvoice.customerId);
    const norm = normalizeInvoice(
      lastSaleInvoice,
      cust,
      lastSaleInvoice.previousBalance,
      { ownerName1: state.data.company.ownerName1, ownerName2: state.data.company.ownerName2 },
    );
    const html = buildInvoicePrintHtml(norm, state.data.company, state.data.invoiceDesign, 'sale', isRTL);
    ...
    <div className="space-y-4">
      <div className={`flex items-center gap-3 flex-wrap ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <button
          ...
          disabled={!lastSaleInvoice}
          title={lastSaleInvoice ? `${isRTL ? 'طباعة آخر فاتورة' : 'Print Last Invoice'}: ${lastSaleInvoice.invoiceNumber}` : ''}
          className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-lg text-sm text-muted-foreground hover:bg-muted/50 hover:text-foreground transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
          <Printer size={14} />
          <span className="hidden sm:inline">{isRTL ? 'طباعة آخر فاتورة' : 'Print Last Invoice'}</span>
        </button>
        <select
          value={lastInvoiceScope}
          onChange={event => setLastInvoiceScope(event.target.value as 'global' | 'customer')}
          className="px-2 py-2 border border-border rounded-lg text-xs bg-card text-muted-foreground"
          title={isRTL ? 'نطاق آخر فاتورة' : 'Last invoice scope'}
        >
          <option value="global">{isRTL ? 'آخر فاتورة بالنظام' : 'Latest system invoice'}</option>
          <option value="customer" disabled={!selectedCustomer}>{isRTL ? 'آخر فاتورة للعميل المختار' : 'Latest selected-customer invoice'}</option>
        </select>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <div className="lg:col-span-3 space-y-4">
            <div className="bg-card rounded-xl border border-border p-4">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={`${t('sales.productSearch')} (F2)`}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {paginatedProducts.map(item => (
                ...
                >
                  <div className="w-full h-20 bg-muted/30 rounded-lg mb-2 flex items-center justify-center overflow-hidden">
                    {item.image ? (
                      <img
                        src={item.image}
          ...
          <div className="lg:col-span-2 bg-card rounded-xl border border-border flex flex-col h-fit sticky top-20">
                ...
                cart.map(ci => (
                  <div key={ci.item.id} className={`flex items-center gap-3 p-2 rounded-lg bg-muted/30 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    {ci.item.image ? (
                      <img
                        src={ci.item.image}
                        alt={isRTL ? ci.item.nameAr || ci.item.name : ci.item.name}
                        className="w-10 h-10 rounded-lg object-cover border border-border flex-shrink-0"
                      />
                    ) : (
                      <div className="w-10 h-10 rounded-lg bg-muted/60 border border-border flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`flex items-center gap-1 mt-0.5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <input
                          ...
                          className="w-16 text-xs font-mono text-primary bg-card border border-border rounded px-1.5 py-0.5 focus:ring-2 focus:ring-primary focus:outline-none"
                        />
                        ...
                        <input
                          ...
                          step="0.01"
                          value={ci.discount}
                          onChange={e => setDiscount(ci.item.id, Number(e.target.value))}
                          onFocus={e => e.target.select()}
                          title={t('common.discount')}
                          className="w-16 text-xs font-mono text-destructive bg-card border border-border rounded px-1.5 py-0.5 focus:ring-2 focus:ring-destructive focus:outline-none"
                        />
            ...
            <div className="px-4 py-3 border-t border-border space-y-2">
              ...
              <p className="pt-1 text-[11px] text-muted-foreground">{isRTL ? 'الإجمالي + الحساب السابق − تحت الحساب = صافي المستحق' : 'Invoice total + previous balance − amount paid = net due'}</p>
            </div>
            ...
            {(paymentMethod === 'cash' || paymentMethod === 'credit') && (
              <div className="px-4 py-2 border-t border-border">
                ...
                />
                <div className={`mt-2 space-y-1 text-xs ${isRTL ? 'text-right' : 'text-left'}`}>
                  <div className="flex justify-between"><span className="text-muted-foreground">{isRTL ? 'تحت الحساب' : 'Under account'}</span><span className="font-mono text-emerald-600">{formatCurrency(Math.min(checkoutPayment, grandTotalDue))}</span></div>
                  <div className="flex justify-between font-semibold"><span>{isRTL ? 'الصافي المستحق' : 'Net due'}</span><span className="font-mono text-primary">{formatCurrency(Math.max(0, grandTotalDue - checkoutPayment))}</span></div>
                </div>
              </div>
            )}
            ...
            <div className="p-4 border-t border-border">
              ...
              <button
                ...
                disabled={(editingInvoiceId ? !can('sales', 'edit') : !can('sales', 'create')) || cart.length === 0}
        ...
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          ...
          {totalPages > 1 && (
            <div className={`px-4 py-3 border-t border-border flex justify-center gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className="px-3 py-1 rounded text-sm hover:bg-accent disabled:opacity-50">«</button>
              {Array.from({ length: totalPages }).map((_, i) => (
                <button key={i} onClick={() => setCurrentPage(i + 1)} className={`px-3 py-1 rounded text-sm ${currentPage === i + 1 ? 'bg-primary text-primary-foreground' : 'hover:bg-accent'}`}>{i + 1}</button>
              ))}
              <button onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className="px-3 py-1 rounded text-sm hover:bg-accent disabled:opacity-50">»</button>
            </div>
          )}
        </div>
      )}
