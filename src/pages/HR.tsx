...
export function HR() {
  ...
  const handleSaveEmployee = () => {
    if (!employeeForm.name) {
      toast.error(t('settings.errInvalidFields'));
      return;
    }
    if (editingEmployee) {
  ...
  const handleCreateAndLinkUser = async () => {
    ...
    if (!res.ok) { toast.error(t('settings.errEmailExists')); return; }
    if (res.user) {
  ...
  const handleSaveAdvance = () => {
    ...
    if (editingAdvance) {
      ...
      const newAdvance = { ...advanceForm, id: generateId('adv'), employeeName: emp?.name || '', status, createdAt: new Date().toISOString() } as EmployeeAdvance;
      ...
      if (safe) {
        dispatch({
          ...
          payload: {
            ...
            category: 'Advances',
            description: `Advance to ${newAdvance.employeeName}`,
            referenceNumber: newAdvance.id,
  ...
  const handleRepayAdvance = (a: EmployeeAdvance) => {
    const amountToRepay = a.amount - (a.repaidAmount || 0);
    dispatch({ type: 'UPDATE_EMPLOYEE_ADVANCE', payload: { ...a, repaidAmount: a.amount, status: 'repaid' } as EmployeeAdvance });
    const safe = state.data.treasuryAccounts.find(acc => acc.type === 'safe');
    if (safe) {
      dispatch({
        type: 'ADD_TRANSACTION',
        payload: {
          ...
          category: 'Advances',
          description: `Advance Repayment from ${a.employeeName}`,
  ...
  const handleGeneratePayroll = () => {
    employees.forEach(emp => {
      ...
      if (!existing && emp.isActive) {
        ...
        const commission = Math.round(baseSalary * (commissionRate / 100));
        const socialInsurance = Math.round(baseSalary * 0.08);

        const safeAttendance = attendanceRecords || [];
        const absentDays = safeAttendance.filter(
          a => a.employeeId === emp.id && 
               a.date.startsWith(`${selectedYear}-${String(selectedMonth + 1).padStart(2, '0')}`) && 
               a.status === 'absent'
        ).length;
        const deductions = Math.round(absentDays * (baseSalary / 30));

        const netSalary = baseSalary + commission - socialInsurance - deductions;
        ...
        const record: PayrollRecord = {
          ...
          baseSalary, commission, bonus: 0, deductions, socialInsurance, netSalary,
  ...
  const handlePayPayroll = (pay: PayrollRecord) => {
    dispatch({ type: 'UPDATE_PAYROLL', payload: { ...pay, status: 'paid' } });
    const safe = state.data.treasuryAccounts.find(a => a.type === 'safe');
    if (safe) {
      dispatch({
        type: 'ADD_TRANSACTION',
        payload: {
          ...
          category: 'Salaries',
          description: `Payroll for ${pay.employeeName} (${pay.month} ${pay.year})`,
                ...
                {monthPayroll.map(pay => (
                  <tr key={pay.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {pay.status === 'approved' && <button onClick={() => handlePayPayroll(pay)} className="px-2 py-1 text-xs bg-emerald-600 text-white dark:bg-emerald-500 rounded hover:bg-emerald-700 dark:hover:bg-emerald-600">{t('hr.pay')}</button>}
                    </td>
                  </tr>
                ))}
              </tbody>
