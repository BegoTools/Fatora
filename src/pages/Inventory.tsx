...
import {
  ...
  Barcode, Download, Upload, Layers, Tag, Ruler, Boxes, Printer, ImagePlus,
} from 'lucide-react';
...
import { exportInventoryToExcel } from '@/services/exportInventory';
...
export function Inventory() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.categoryId === NEW_CATEGORY_VALUE && !newCategoryName.trim() && !newCategoryNameAr.trim()) {
      toast.warning(isRTL ? 'يرجى كتابة اسم الفئة الجديدة' : 'Please enter the new category name');
      return;
    }
  ...
  const exportExcel = () => {
    exportInventoryToExcel({
      items: filteredItems, // uses current filters
      categories,
                    ...
                    <td className="px-4 py-3">
                      <div className={`flex items-center gap-2.5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        ...
                        {item.image ? (
                          ...
                          <div className="w-9 h-9 rounded-lg bg-muted/50 flex items-center justify-center flex-shrink-0 border border-border">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">
                  {isRTL ? 'صورة الصنف (اختياري)' : 'Item Image (optional)'}
                </label>
                <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  {formData.image ? (
                    ...
                    <div className="w-20 h-20 rounded-xl bg-muted/40 border-2 border-dashed border-border flex items-center justify-center">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 border border-border rounded-lg text-xs text-muted-foreground hover:bg-accent cursor-pointer"
                    >
                      <ImagePlus size={14} />
                    ...
                    <p className="text-[10px] text-muted-foreground/60">
                      {isRTL ? 'PNG, JPG حتى 2 ميجا' : 'PNG, JPG up to 2 MB'}
                    </p>
                  </div>
                </div>
              </div>
