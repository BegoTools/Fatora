...
import { Plus, Search, Pencil, Trash2, X, Wallet, FileText, ArrowLeft, ArrowRight, Eye, Download, Printer } from 'lucide-react';
import { useApp } from '@/context/AppContext';
import { useAuth } from '@/context/AuthContext';
import { generateId } from '@/db';
import type { Customer, AuditLogEntry, CustomerAdjustment, SaleInvoice } from '@/types';
import { InvoiceView } from '@/components/invoice/InvoiceView';
import { exportCustomerStatement } from '@/services/exportReports';
...
export function Customers() {
  ...
  const saveCustomer = () => {
    ...
    if (editing) {
      ...
      const audit: AuditLogEntry = {
        id: generateId('audit'), action: 'customer.update', entityType: 'Customer', entityId: editing.id,
        description: `Edit Customer: ${updated.name}`,
        before: JSON.stringify(editing),
  ...
  const saveAdjustment = () => {
    ...
    if (!canAdjustCustomerBalance) return;
  ...
  if (accountCustomer) {
    ...
    let running = 0;
    const statementRows = rows.map(row => ({ ...row, balance: (running += row.debit - row.credit) }));
    const filteredStatementRows = statementRows.filter(row =>
      (!statementFrom || row.date >= statementFrom) &&
      (!statementTo || row.date <= `${statementTo}T23:59:59`),
    );
    const exportStatementPdf = () => printReport({
      title: `${t('customers.accountStatement')} — ${isRTL ? accountCustomer.nameAr : accountCustomer.name}`,
      subtitle: statementFrom || statementTo ? `${statementFrom || '…'} — ${statementTo || '…'}` : undefined,
      direction: isRTL ? 'rtl' : 'ltr',
      columns: [t('common.date'), t('common.type'), t('sales.invoiceNumber'), t('customers.debit'), t('customers.credit'), t('common.balance'), t('customers.adjustReason')],
      rows: filteredStatementRows.map(row => [new Date(row.date).toLocaleDateString(), row.type, row.ref, formatCurrency(row.debit), formatCurrency(row.credit), formatCurrency(row.balance), row.note]),
    });
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          <div className={`px-4 py-3 border-b border-border flex flex-wrap gap-2 items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
            <span className="font-semibold text-foreground">{t('customers.accountStatement')}</span>
            <div className={`flex flex-wrap gap-2 items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
              <input type="date" value={statementFrom} onChange={event => setStatementFrom(event.target.value)} aria-label={t('common.from')} className="px-2 py-1 border border-border rounded text-xs bg-card" />
              <input type="date" value={statementTo} onChange={event => setStatementTo(event.target.value)} aria-label={t('common.to')} className="px-2 py-1 border border-border rounded text-xs bg-card" />
              <button onClick={() => exportCustomerStatement(isRTL ? accountCustomer.nameAr : accountCustomer.name, filteredStatementRows.map(row => ({ ...row, reference: row.ref, reason: row.note })), state.data.company.currencySymbol)} className="p-1.5 rounded border border-border text-muted-foreground hover:bg-muted" title="Excel"><Download size={14} /></button>
              <button onClick={exportStatementPdf} className="p-1.5 rounded border border-border text-muted-foreground hover:bg-muted" title="PDF"><Printer size={14} /></button>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                ...
                {filteredStatementRows.map((r, idx) => {
                  return (
                    <tr key={idx} className="hover:bg-muted/40">
                      ...
                      <td className="px-3 py-2 font-mono text-emerald-600">{r.credit ? formatCurrency(r.credit) : '-'}</td>
                       <td className="px-3 py-2 font-mono font-medium">{formatCurrency(r.balance)}</td>
