...
export function Treasury() {
  ...
  const totalBalance = round2(treasuryAccounts.reduce((sum, a) => sum + (a.balance ?? 0), 0));
  ...
  const handleTransfer = () => {
    ...
    dispatch({ type: 'ADD_MULTIPLE_TRANSACTIONS', payload: [outTransaction, inTransaction] });
  ...
  const totalIncome = round2(transactions.filter(t => t.type === 'income').reduce((sum, t) => sum + (t.amount ?? 0), 0));
  const totalExpense = round2(transactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + (t.amount ?? 0), 0));
