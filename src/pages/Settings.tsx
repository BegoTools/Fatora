...
import { Save, Upload, Download, RotateCcw, Building2, Globe, Database, Users, Shield, X, Plus, Trash2, Sparkles, Eye, EyeOff, Lock, Pencil, ReceiptText, Image as ImageIcon, Store, DollarSign, RefreshCw, GripVertical, Cloud } from 'lucide-react';
...
import type { User, UserRole, InvoiceDesign, InvoiceLayoutSettings, InvoiceSectionId, InvoiceCustomField } from '@/types';
...
import { InvoiceSpreadsheetDesigner } from '@/components/invoice/InvoiceSpreadsheetDesigner';
import { resolveInvoiceSpreadsheetTemplate } from '@/components/invoice/invoiceSpreadsheetTemplate';
import { aiManager } from '@/services/ai';
import { SectorProfileModal } from '@/components/sectors/SectorProfileModal';
import { DatabaseMaintenanceModal } from '@/components/database/DatabaseMaintenanceModal';
import { CashAuditModal } from '@/components/cashAudit/CashAuditModal';
...
export function Settings() {
  ...
  const [tab, setTab] = useState<'company' | 'invoice' | 'users' | 'roles' | 'backup' | 'language' | 'ai' | 'sync'>('company');
  const [companyForm, setCompanyForm] = useState({ ...company });
  const [designForm, setDesignForm] = useState<InvoiceDesign>(invoiceDesign);
  const [draggedSection, setDraggedSection] = useState<InvoiceSectionId | null>(null);
  const [selectedSection, setSelectedSection] = useState<InvoiceSectionId>('header');
  ...
  const [showDbMaintenanceModal, setShowDbMaintenanceModal] = useState(false);
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) { toast.warning(t('settings.logoInvalid')); return; }
    if (file.size > 1024 * 1024) { toast.warning(t('settings.logoTooLarge')); return; }
  ...
  const invoiceLayout: InvoiceLayoutSettings = {
    sectionOrder: ['header', 'details', 'items', 'totals', 'footer'],
    fontFamily: 'Arial, sans-serif', baseFontSize: 13, headerHeight: 112, logoWidth: 150,
    tableFontSize: 13, showFooter: true, tableHeaderColor: '#000000', totalsLabelColor: '#434343',
    ...designForm.layout,
  };
  const updateInvoiceLayout = (patch: Partial<InvoiceLayoutSettings>) => {
    setDesignForm(previous => ({ ...previous, layout: { ...invoiceLayout, ...patch } }));
  };
  const updateSectionStyleFor = (section: InvoiceSectionId, patch: NonNullable<InvoiceLayoutSettings['sectionStyles']>[InvoiceSectionId]) => {
    setDesignForm(previous => ({
      ...previous,
      layout: {
        ...invoiceLayout,
        sectionStyles: {
          ...invoiceLayout.sectionStyles,
          [section]: { ...invoiceLayout.sectionStyles?.[section], ...patch },
        },
      },
    }));
  };
  const updateSectionStyle = (patch: NonNullable<InvoiceLayoutSettings['sectionStyles']>[InvoiceSectionId]) => updateSectionStyleFor(selectedSection, patch);
  const addCustomField = () => setDesignForm(previous => ({
    ...previous,
    customFields: [...(previous.customFields || []), { labelEn: 'New field', labelAr: 'حقل جديد', value: '' }],
  }));
  const updateCustomField = (index: number, patch: Partial<InvoiceCustomField>) => setDesignForm(previous => ({
    ...previous,
    customFields: (previous.customFields || []).map((field, i) => i === index ? { ...field, ...patch } : field),
  }));
  const removeCustomField = (index: number) => setDesignForm(previous => ({
    ...previous,
    customFields: (previous.customFields || []).filter((_, i) => i !== index),
  }));
  const sectionLabels: Record<InvoiceSectionId, string> = {
    header: isRTL ? 'رأس الفاتورة' : 'Invoice header',
    details: isRTL ? 'بيانات الفاتورة' : 'Invoice details',
    items: isRTL ? 'جدول الأصناف' : 'Items table',
    totals: isRTL ? 'الإجماليات' : 'Totals',
    footer: isRTL ? 'تذييل الفاتورة' : 'Invoice footer',
  };
  const moveSection = (target: InvoiceSectionId) => {
    if (!draggedSection || draggedSection === target) return;
    const order = [...invoiceLayout.sectionOrder];
    const from = order.indexOf(draggedSection);
    const to = order.indexOf(target);
    order.splice(from, 1);
    order.splice(to, 0, draggedSection);
    updateInvoiceLayout({ sectionOrder: order });
  ...
  const handleResetToZero = () => {
    if (window.confirm(t('settings.resetToZeroConfirm'))) {
      ...
      toast.success(t('settings.resetToZeroDone'));
  ...
  const handleDeleteUser = async (u: User) => {
    if (u.role === 'owner') { toast.warning(t('settings.cannotDeleteOwner')); return; }
    if (u.id === currentUser?.id) { toast.warning(t('settings.cannotDeleteSelf')); return; }
  ...
  const handleDeleteRole = async (r: RoleDefinition) => {
    if (r.isSystem) { toast.warning(t('settings.errRoleProtected')); return; }
    const inUse = users.some(u => u.role === r.id);
    if (inUse) { toast.warning(t('settings.errRoleInUse')); return; }
    if (window.confirm(t('notifications.confirmDelete'))) {
  ...
  const tabs = [
    ...
    { id: 'sync', label: isRTL ? 'تخزين سحابي' : 'Cloud Sync', icon: <Cloud size={16} /> },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === 'invoice' && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            ...
            {/* Visual invoice builder: the values are persisted in InvoiceDesign.layout. */}
            <div className="bg-card rounded-xl border border-border p-5 space-y-4">
              <div>
                <h3 className="text-sm font-semibold text-foreground">{isRTL ? 'مصمم شكل الفاتورة' : 'Visual invoice designer'}</h3>
                <p className="text-xs text-muted-foreground mt-1">{isRTL ? 'اسحب الأقسام لتغيير ترتيبها، ثم عدّل المقاسات والخطوط.' : 'Drag sections to reorder them, then adjust sizing and typography.'}</p>
              </div>
              <div className="space-y-1">
                {invoiceLayout.sectionOrder.map(section => (
                  <div key={section} draggable onClick={() => setSelectedSection(section)} onDragStart={() => setDraggedSection(section)} onDragOver={event => event.preventDefault()} onDrop={() => moveSection(section)}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-grab ${draggedSection === section || selectedSection === section ? 'border-primary bg-primary/10' : 'border-border bg-muted/20'}`}>
              ...
              <div className="rounded-lg border border-dashed border-primary/50 bg-primary/5 p-3">
                <p className="text-xs font-semibold text-foreground mb-2">{isRTL ? `خصائص: ${sectionLabels[selectedSection]}` : `Properties: ${sectionLabels[selectedSection]}`}</p>
                <div className="grid grid-cols-2 gap-3">
                  <label className="text-xs text-muted-foreground">{isRTL ? 'العرض' : 'Width'}
                    <input type="range" min="30" max="100" value={invoiceLayout.sectionStyles?.[selectedSection]?.width ?? 100} onChange={event => updateSectionStyle({ width: Number(event.target.value) })} className="mt-2 w-full" />
                  </label>
                  <label className="text-xs text-muted-foreground">{isRTL ? 'المسافة الداخلية' : 'Padding'}
                    <input type="range" min="0" max="32" value={invoiceLayout.sectionStyles?.[selectedSection]?.padding ?? 0} onChange={event => updateSectionStyle({ padding: Number(event.target.value) })} className="mt-2 w-full" />
                  </label>
                  <label className="text-xs text-muted-foreground">{isRTL ? 'لون الخلفية' : 'Background'}
                    <input type="color" value={invoiceLayout.sectionStyles?.[selectedSection]?.backgroundColor || '#ffffff'} onChange={event => updateSectionStyle({ backgroundColor: event.target.value })} className="mt-1 block h-9 w-full rounded border border-border" />
                  </label>
                  <label className="text-xs text-muted-foreground">{isRTL ? 'لون النص' : 'Text colour'}
                    <input type="color" value={invoiceLayout.sectionStyles?.[selectedSection]?.textColor || '#000000'} onChange={event => updateSectionStyle({ textColor: event.target.value })} className="mt-1 block h-9 w-full rounded border border-border" />
                  </label>
                </div>
                <div className={`flex gap-4 mt-3 text-xs ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <label className="flex items-center gap-1.5"><input type="checkbox" checked={!invoiceLayout.sectionStyles?.[selectedSection]?.hidden} onChange={event => updateSectionStyle({ hidden: !event.target.checked })} /> {isRTL ? 'إظهار القسم' : 'Show section'}</label>
                  <select value={invoiceLayout.sectionStyles?.[selectedSection]?.align || 'start'} onChange={event => updateSectionStyle({ align: event.target.value as 'start' | 'center' | 'end' })} className="border border-border rounded px-2 py-1 bg-card">
                    <option value="start">{isRTL ? 'بداية' : 'Start'}</option><option value="center">{isRTL ? 'منتصف' : 'Center'}</option><option value="end">{isRTL ? 'نهاية' : 'End'}</option>
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label className="text-xs text-muted-foreground">{isRTL ? 'نوع الخط' : 'Font family'}
                  <select value={invoiceLayout.fontFamily} onChange={event => updateInvoiceLayout({ fontFamily: event.target.value })} className="mt-1 w-full px-3 py-2 border border-border rounded-lg bg-card text-sm">
                    <option value="Arial, sans-serif">Arial</option><option value="Tahoma, sans-serif">Tahoma</option><option value="'Times New Roman', serif">Times New Roman</option><option value="'Courier New', monospace">Courier New</option>
                  </select>
                </label>
                <label className="text-xs text-muted-foreground">{isRTL ? 'حجم الخط العام' : 'Base font size'}
                  <input type="range" min="10" max="18" value={invoiceLayout.baseFontSize} onChange={event => updateInvoiceLayout({ baseFontSize: Number(event.target.value) })} className="mt-2 w-full" />
                </label>
                <label className="text-xs text-muted-foreground">{isRTL ? 'ارتفاع الرأس' : 'Header height'}
                  <input type="range" min="70" max="220" value={invoiceLayout.headerHeight} onChange={event => updateInvoiceLayout({ headerHeight: Number(event.target.value) })} className="mt-2 w-full" />
                </label>
                <label className="text-xs text-muted-foreground">{isRTL ? 'عرض مربع الشعار' : 'Logo box width'}
                  <input type="range" min="80" max="300" value={invoiceLayout.logoWidth} onChange={event => updateInvoiceLayout({ logoWidth: Number(event.target.value) })} className="mt-2 w-full" />
                </label>
                <label className="text-xs text-muted-foreground">{isRTL ? 'لون رأس جدول الأصناف' : 'Items header colour'}
                  <input type="color" value={invoiceLayout.tableHeaderColor} onChange={event => updateInvoiceLayout({ tableHeaderColor: event.target.value })} className="mt-1 block h-9 w-full rounded border border-border" />
                </label>
                <label className="text-xs text-muted-foreground">{isRTL ? 'لون عناوين الإجماليات' : 'Totals label colour'}
                  <input type="color" value={invoiceLayout.totalsLabelColor} onChange={event => updateInvoiceLayout({ totalsLabelColor: event.target.value })} className="mt-1 block h-9 w-full rounded border border-border" />
                </label>
              </div>
              <label className={`flex items-center gap-2 text-sm text-muted-foreground ${isRTL ? 'flex-row-reverse' : ''}`}>
                <input type="checkbox" checked={invoiceLayout.showFooter} onChange={event => updateInvoiceLayout({ showFooter: event.target.checked })} /> {isRTL ? 'إظهار تذييل الفاتورة' : 'Show invoice footer'}
              </label>
            </div>
            ...
            <div className="bg-card rounded-xl border border-border p-5">
              ...
              <div className="space-y-3">
                ...
                {(designForm.customFields || []).map((field, index) => (
                  <div key={`custom-${index}`} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_auto] gap-2">
                    <input value={field.labelEn} onChange={e => updateCustomField(index, { labelEn: e.target.value })} placeholder="Field name EN" className="px-3 py-2 border border-border rounded-lg text-sm" />
                    <input value={field.labelAr} onChange={e => updateCustomField(index, { labelAr: e.target.value })} dir="rtl" placeholder="اسم الحقل" className="px-3 py-2 border border-border rounded-lg text-sm" />
                    <input value={field.value} onChange={e => updateCustomField(index, { value: e.target.value })} placeholder={t('settings.value')} className="px-3 py-2 border border-border rounded-lg text-sm" />
                    <button type="button" onClick={() => removeCustomField(index)} className="p-2 rounded-lg text-destructive hover:bg-destructive/10" title={t('common.delete')}><Trash2 size={16} /></button>
                  </div>
                ))}
                <button type="button" onClick={addCustomField} className="inline-flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border border-primary text-primary hover:bg-primary/10"><Plus size={15} /> {isRTL ? 'إضافة خانة' : 'Add field'}</button>
              </div>
            </div>
          ...
          {/* Google Sheets-style invoice board */}
          <div className="xl:sticky xl:top-4 self-start">
            <p className="text-xs text-muted-foreground mb-2">{isRTL ? 'ورقة تصميم الفاتورة — مثل Google Sheets' : 'Invoice design sheet — Google Sheets style'}</p>
            <InvoiceSpreadsheetDesigner value={resolveInvoiceSpreadsheetTemplate(designForm.spreadsheet)} onChange={spreadsheet => setDesignForm(previous => ({ ...previous, spreadsheet }))} />
          </div>
        </div>
      )}
        ...
        <div className="bg-card rounded-xl border border-border p-6 max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-muted/50 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
              ...
              <p>{t('settings.aiModel')}: <span className="font-medium text-foreground">gemini-2.5-flash</span></p>
              <p>
        ...
        <div className="space-y-4 max-w-md">
          <div className="bg-card rounded-xl border border-border p-6">
            <h3 className="text-lg font-semibold text-foreground mb-2 flex items-center gap-2">
              ...
              <span>تهيئة قطاع النشاط التجاري (Vertical Templates)</span>
            </h3>
            <p className="text-xs text-muted-foreground mb-4">
              تخصيص الحقول والتنبهات والواجهة تلقائيًا حسب قطاع عملك (ملابس، سوبرماركت، صيانة، إلكترونيات...)
            </p>
            <button onClick={() => setShowSectorModal(true)} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">
              <Store size={16} /> اختيار وتخصيص القطاع
            </button>
          </div>
          ...
          <div className="bg-card rounded-xl border border-border p-6">
            <h3 className="text-lg font-semibold text-foreground mb-2 flex items-center gap-2">
              ...
              <span>جرد ومطابقة الخزينة والنقدية</span>
            </h3>
            <p className="text-xs text-muted-foreground mb-4">
              مقارنة النقدية الفعلية بالدفتري وتسوية العجز أو الزيادة آليًا عبر قيد اليومية
            </p>
            <button onClick={() => setShowCashAuditModal(true)} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700">
              <DollarSign size={16} /> فتح شاشة جرد الخزينة
            </button>
          </div>
          ...
          <div className="bg-card rounded-xl border border-border p-6">
            <h3 className="text-lg font-semibold text-foreground mb-2 flex items-center gap-2">
              ...
              <span>أدوات صيانة قاعدة البيانات والسنة المالية</span>
            </h3>
            <p className="text-xs text-muted-foreground mb-4">
              مسح الفواتير والحركات لبداية سنة جديدة مع الحفاظ على بطاقات الأصناف والعملاء والموردين
            </p>
            <button onClick={() => setShowDbMaintenanceModal(true)} className="flex items-center gap-2 px-4 py-2 border border-amber-600 text-amber-700 rounded-lg text-sm font-medium hover:bg-amber-50">
              <RefreshCw size={16} /> أدوات الصيانة والتنظيف
            </button>
          </div>
      ...
      {/* Role Modal */}
      {showRoleModal && (
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 overflow-y-auto">
      ...
      {/* Modals */}
      <SectorProfileModal isOpen={showSectorModal} onClose={() => setShowSectorModal(false)} />
      <DatabaseMaintenanceModal isOpen={showDbMaintenanceModal} onClose={() => setShowDbMaintenanceModal(false)} />
      <CashAuditModal isOpen={showCashAuditModal} onClose={() => setShowCashAuditModal(false)} />
    </div>
  );
}
