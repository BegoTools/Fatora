...
export function Dashboard() {
  ...
  const yesterdaySales = salesInvoices.filter(s => s.createdAt.startsWith(yesterday));
  const yesterdayRevenue = yesterdaySales.reduce((sum, s) => sum + s.total, 0);
  
  const revenueChangePct = yesterdayRevenue === 0 
    ? (todayRevenue > 0 ? '+100%' : '0%') 
    : `${todayRevenue >= yesterdayRevenue ? '+' : ''}${Math.round(((todayRevenue - yesterdayRevenue) / yesterdayRevenue) * 100)}%`;
  ...
  const unreadAlerts = notifications.filter(n => !n.isRead && n.type === 'warning').length;
    return { lowStockItems, outOfStockItems, activeItems, todaySales, todayRevenue, yesterdaySales, yesterdayRevenue, revenueChangePct, revenueTrend, activeStaff, unreadAlerts };
  }, [items, salesInvoices, employees, notifications]);

  const { lowStockItems, outOfStockItems, activeItems, todayRevenue, revenueChangePct, revenueTrend, activeStaff, unreadAlerts } = dashboardStats;
  ...
  const chartData = useMemo(() => {
    if (revenueRange === 'week') {
      ...
      return last4Weeks.map((date, i) => {
        ...
        const weekSales = salesInvoices.filter(s => {
          const sDate = new Date(s.createdAt);
          return sDate >= date && sDate < weekEnd;
        });
        return {
          day: `W${i + 1}`,
      ...
      return last12Months.map(date => {
        const monthSales = salesInvoices.filter(s => {
          const sDate = new Date(s.createdAt);
          return sDate.getMonth() === date.getMonth() && sDate.getFullYear() === date.getFullYear();
        });
        return {
          day: date.toLocaleDateString(isRTL ? 'ar-EG' : 'en-US', { month: 'short' }),
          sales: monthSales.reduce((sum, s) => sum + s.total, 0),
          count: monthSales.length,
        };
      });
    }
  }, [revenueRange, salesInvoices, isRTL]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-card rounded-xl border border-border p-5">
          <div className={`flex items-center justify-between mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <select 
              className="text-xs border border-border rounded-lg px-2 py-1 bg-card focus:outline-none focus:ring-2 focus:ring-primary"
              value={revenueRange}
