...
export function Expenses() {
  ...
  const handleSave = () => {
    ...
    if (!amount || amount <= 0) { toast.warning(t('expenses.invalidAmount')); return; }
    if (!form.accountId) { toast.warning(t('expenses.selectAccount')); return; }
    ...
    if (editingId) {
      dispatch({ type: 'UPDATE_TRANSACTION', payload: newTr });
    } else {
