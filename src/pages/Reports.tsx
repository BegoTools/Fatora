...
import { Download, BarChart3, TrendingUp, Package, Users, Calendar, Wrench } from 'lucide-react';

import { useApp } from '@/context/AppContext';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } from 'recharts';
import { MaintenanceView } from '@/components/maintenance/MaintenanceView';
import { exportCustomerDebtsReport, exportInventoryReport, exportPurchasesReport, exportSalesReport } from '@/services/exportReports';
...
export function Reports() {
  ...
  const [reportType, setReportType] = useState<'sales' | 'inventory' | 'financial' | 'hr' | 'maintenance'>('sales');
  ...
  const exportExcel = () => {
    ...
    if (reportType === 'sales') exportSalesReport(salesInvoices, range, currency);
    else if (reportType === 'inventory') exportInventoryReport(items, currency);
    else if (reportType === 'financial') exportPurchasesReport(purchaseInvoices, range, currency);
    else exportCustomerDebtsReport(state.data.customers, salesInvoices, currency);
  };
  ...
  const exportPdf = () => {
    ...
    if (reportType === 'inventory') {
      printReport({ title: t('reports.stockReport'), subtitle, direction: isRTL ? 'rtl' : 'ltr',
        columns: [t('common.name'), t('common.quantity'), t('inventory.fields.purchasePrice'), t('inventory.fields.salePrice')],
        rows: items.map(item => [isRTL ? item.nameAr || item.name : item.name, item.stockQuantity, formatCurrency(item.purchasePrice), formatCurrency(item.salePrice)]) });
      return;
    }
    if (reportType === 'sales') {
      printReport({ title: t('reports.salesSummary'), subtitle, direction: isRTL ? 'rtl' : 'ltr',
        columns: [t('sales.invoiceNumber'), t('common.customer'), t('common.date'), t('common.total'), t('common.paid'), t('common.remaining')],
        rows: filteredSales.map(invoice => [invoice.invoiceNumber, invoice.customerName, new Date(invoice.createdAt).toLocaleDateString(), `${invoice.total} ${currency}`, `${invoice.paid} ${currency}`, `${invoice.remaining} ${currency}`]) });
      return;
    }
    printReport({ title: t('reports.incomeStatement'), subtitle, direction: isRTL ? 'rtl' : 'ltr',
      columns: [t('common.description'), t('common.amount')],
      rows: [[t('common.revenue'), formatCurrency(filteredSales.reduce((sum, invoice) => sum + invoice.total, 0))], [t('common.purchases'), formatCurrency(filteredPurchases.reduce((sum, invoice) => sum + invoice.total, 0))]] });
  };
  ...
  const reportCards = [
    ...
    { id: 'maintenance', label: t('reports.maintenance'), icon: <Wrench size={18} />, color: 'bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400' },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`bg-card rounded-xl border border-border p-4 flex flex-wrap items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <div className={`flex gap-2 ${isRTL ? 'mr-auto flex-row-reverse' : 'ml-auto'}`}>
          <button onClick={exportExcel} className="flex items-center gap-1.5 px-3 py-2 border border-border rounded-lg text-sm text-muted-foreground hover:bg-muted/50">
            <Download size={14} /> Excel
          </button>
          <button onClick={exportPdf} className="flex items-center gap-1.5 px-3 py-2 bg-primary text-primary-foreground rounded-lg text-sm hover:bg-primary/90">
